...
class Test_Enigma(TestCase):
    ...
    #     self.IV.update({"start_position": "Z", "ring_setting": 19, "position": 3})
    ...
    def test_m4_Beta_VI_II_III_B_Thin_AAA(self):
        settings = Settings()
        data = Data()
        data.set_machine("m4")
        beta = data.get_rotor("beta")
        vi = data.get_rotor("vi")
        ii = data.get_rotor("ii")
        iii = data.get_rotor("iii")
        beta.update({"start_position": "D", "ring_setting": 11, "position": 1})
        vi.update({"start_position": "Y", "ring_setting": 20, "position": 2})
        ii.update({"start_position": "A", "ring_setting": 14, "position": 3})
        iii.update({"start_position": "A", "ring_setting": 3, "position": 4})
        settings.add_rotors([beta, vi, ii, iii])
        settings.set_reflector(**data.get_reflector("b-thin"))
        self.assertEqual("VRQEK", Enigma(settings=settings).parse("AAAAA"))

    def test_ring_setting_and_turnover_interaction(self):
        self.I.update({"start_position": "Q", "ring_setting": 1, "position": 1})
        self.II.update({"start_position": "E",  "ring_setting": 1, "position": 2})
        self.III.update({"start_position": "V", "ring_setting": 2, "position": 3})
        self.settings.set_reflector(**self.B)
        self.settings.add_rotors([self.I, self.II, self.III])
        self.assertEqual("HLNPJ", Enigma(settings=self.settings).parse("AAAAA"))
